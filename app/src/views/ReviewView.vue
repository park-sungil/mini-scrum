...
const currentSprint = computed(() => store.sprints.find(s => s.id === selectedSprintId.value))
const sprintTasks = computed(() => store.tasks.filter(t => t.sprint_id === selectedSprintId.value))
const completedTasks = computed(() => sprintTasks.value.filter(t => t.status === 'done'))
const incompleteTasks = computed(() => sprintTasks.value.filter(t => t.status !== 'done'))
const review = computed(() => store.reviews.find(r => r.sprint_id === selectedSprintId.value))
...
async function saveReview() {
  ...
  if (review.value) await api.updateReview(review.value.id, data)
...
<template>
  <div>
    <div class="flex items-center justify-between mb-8">
      <div>
        <h1 class="font-display text-4xl" style="color: var(--text)">Weekly Review</h1>
    ...
    <div class="space-y-5 animate-in">
      <div class="card p-6" style="border-left: 3px solid var(--accent)">
        ...
        <p class="text-sm leading-relaxed" style="color: var(--text)">{{ currentSprint?.goal || '목표가 설정되지 않았습니다.' }}</p>
        <div class="font-mono text-xs mt-2" style="color: var(--text-muted)">{{ currentSprint?.start_date }} ~ {{ currentSprint?.end_date }}</div>
      </div>
      ...
      <div class="grid grid-cols-2 gap-5">
        <div class="card p-6">
          ...
          <div v-if="!completedTasks.length" class="text-xs" style="color: var(--text-muted)">완료된 업무가 없습니다.</div>
          <ul class="space-y-2.5">
            <li v-for="task in completedTasks" :key="task.id" class="relative group">
              <div class="flex items-center gap-2 text-sm">
                <svg class="w-4 h-4 flex-shrink-0" style="color: var(--success)" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/></svg>
      ...
      <div class="card p-6">
        <div class="flex items-center justify-between mb-5">
          ...
          <button v-if="!editing" @click="startEdit" class="btn-ghost" style="padding: 5px 12px">{{ review ? '수정' : '작성하기' }}</button>
        </div>
        ...
        <form v-if="editing" @submit.prevent="saveReview" class="space-y-4">
          <div><label class="label" style="color: var(--danger)">미완료 사유</label><textarea v-model="form.incomplete_reason" rows="3" class="input" placeholder="완료하지 못한 업무의 사유를 작성하세요"></textarea></div>
          <div><label class="label" style="color: var(--warning)">이슈 / 블로커</label><textarea v-model="form.blockers" rows="3" class="input" placeholder="진행을 막는 문제가 있다면 작성하세요"></textarea></div>
          <div><label class="label" style="color: var(--info)">다음 주 계획</label><textarea v-model="form.next_plan" rows="3" class="input" placeholder="다음 스프린트에 할 일을 작성하세요"></textarea></div>
